...
2. **"404 Not Found" or "npm ERR! code E404" when installing**
   - The `@noedgeai` scope is hosted on GitHub Packages, not the default npm registry.
   - Configure the registry first: `npm config set @noedgeai:registry=https://npm.pkg.github.com`
   - Then retry: `npm i -g @noedgeai/doc2x-cli`
...
4. **Client mode failure — desktop app not running**
...
6. **"File not found or not readable: <path>"** (exit code 3)
   - Check the file path is correct and the file exists.

7. **"File too large: X MB (max Y for Z)"** (exit code 3)
   - PDFs must be under 300 MB, images under 3 MB.

8. **"File is empty: <path>"** (exit code 3)
   - The input file has 0 bytes. Check if the file is corrupted.

9. **"Unsupported image format: .webp"** (exit code 3)
...
10. **"doc2x is mandatory in --image-models and cannot be removed."** (exit code 1)
   - The `doc2x` OCR model cannot be removed from the image-models list.

11. **"Invalid <name>: "<value>". Valid values: <list>"** (exit code 1)
...
13. **"No input files or directories specified."** (exit code 1)
    - Batch command requires at least one input file or directory.

### Subscription and Quota Issues

14. **"Model "<name>" requires a subscription."** (exit code 2)
...
15. **"Insufficient quota: no available pages remaining (free: 0, subscription: 0)."** (exit code 2)
